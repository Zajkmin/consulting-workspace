...
class SupabaseProjectRepository implements ProjectRepository {
  ...
  async create(project:Project):Promise<Project>{
    const clientId=project.clientId?.trim() ?? null;
    if(clientId&&(await this.store.byId("clients",clientId))===undefined)throw new Error("El cliente seleccionado no existe.");
    const areas=[...new Set(project.areas??[project.area])].map(name=>({id:randomUUID(),name}));
    await this.store.rpc("save_project_with_areas",{p_record:{id:project.id,client_id:clientId,name:project.name,color:project.color,active:project.active,primary_area:project.area},p_areas:areas,p_manager_profile_id:null,p_expected_updated_at:null});
    return project;
  }
  async update(project:Project):Promise<Project>{
    ...
    const clientId=project.clientId?.trim() ?? null;
    if(clientId&&(await this.store.byId("clients",clientId))===undefined)throw new Error("El cliente seleccionado no existe.");
    await this.store.rpc("save_project_with_areas",{p_record:{id:project.id,client_id:clientId,name:project.name,color:project.color,active:project.active,primary_area:project.area},p_areas:areas,p_manager_profile_id:null,p_expected_updated_at:project.revision??null});
    return project;
  }
