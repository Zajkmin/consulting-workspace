...
export const listMappers = {
  ...
  users:(items:SharePointListItem[])=>mapItems<User>("CW_Users",items,f=>({id:requiredText(f,"AppId"),name:requiredText(f,"Title"),email:requiredText(f,"Email").toLowerCase(),initials:requiredText(f,"Initials"),role:oneOf(f,"Role",["admin","usuario"] as const),assignedProjectIds:[],editableProjectIds:[],permissions:{manageUsers:boolean(f,"ManageUsers"),manageProjects:boolean(f,"ManageProjects"),manageSchedule:boolean(f,"ManageSchedule")},active:boolean(f,"IsActive"),entraObjectId:optionalText(f,"EntraObjectId")})),
