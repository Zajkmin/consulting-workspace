...
export interface TaskRecord { id:string; projectId:string; initiativeId:string; versionId:string; title:string; description:string; priority:"Alta"|"Media"|"Baja"; status:TaskStatus; deadline:string; estimatedMinutes:number; splittable:boolean; progress:number; assignedUserId:string; assignedProfileId?: string; revision?:string }
export interface SubtaskRecord extends Omit<Subtask,"assignedTo"|"assignedProfileId"> { taskId:string; assignedUserId:string; assignedProfileId?: string }
...
const optionalBoolean = (f:Record<string,unknown>, key:string) => f[key] === undefined || f[key] === null || f[key] === "" ? false : boolean(f, key);
...
const optionalDate = (f:Record<string,unknown>, key:string) => {const v=optionalText(f,key);if(!v)return "";if(!/^\d{4}-\d{2}-\d{2}/.test(v)||Number.isNaN(Date.parse(v)))throw new InvalidFieldError(`${key} debe ser una fecha válida`);return v.slice(0,10)};
...
export const listMappers = {
  projects:(items:SharePointListItem[])=>mapItems<Project>("CW_Projects",items,f=>{const area=requiredText(f,"PrimaryArea");return{id:requiredText(f,"AppId"),clientId:optionalText(f,"ClientId") ?? null,name:requiredText(f,"Title"),color:requiredText(f,"ColorHex"),area,areas:[],active:boolean(f,"IsActive")}}),
  ...
  initiatives:(items:SharePointListItem[])=>mapItems<InitiativeRecord>("CW_Initiatives",items,f=>{const ownerUserId=requiredText(f,"OwnerUserId"),stored=optionalText(f,"OwnerUserIds")?.split(",").map(v=>v.trim()).filter(Boolean)??[];return{id:requiredText(f,"AppId"),projectId:requiredText(f,"ProjectId"),areaId:requiredText(f,"AreaId"),name:requiredText(f,"Title"),description:optionalText(f,"Description"),successCriteria:optionalText(f,"SuccessCriteria"),status:oneOf(f,"Status",statuses),ownerUserId,ownerUserIds:[...new Set([ownerUserId,...stored])],startDate:date(f,"StartDate"),deadline:optionalDate(f,"Deadline"),impact:oneOf(f,"Impact",["Alto","Medio","Bajo"] as const)}}),
  versions:(items:SharePointListItem[])=>mapItems<VersionRecord>("CW_Versions",items,f=>({id:requiredText(f,"AppId"),initiativeId:requiredText(f,"InitiativeId"),code:requiredText(f,"Code"),name:requiredText(f,"Title"),status:oneOf(f,"Status",statuses),ownerUserId:requiredText(f,"OwnerUserId"),startDate:date(f,"StartDate"),deadline:date(f,"Deadline"),validated:optionalBoolean(f,"Validated")})),
  tasks:(items:SharePointListItem[])=>mapItems<TaskRecord>("CW_Tasks",items,f=>{const status=oneOf(f,"Status",statuses);return{id:requiredText(f,"AppId"),projectId:requiredText(f,"ProjectId"),initiativeId:requiredText(f,"InitiativeId"),versionId:requiredText(f,"VersionId"),title:requiredText(f,"Title"),description:optionalText(f,"Description")??"",priority:oneOf(f,"Priority",["Alta","Media","Baja"] as const),status,deadline:date(f,"Deadline"),estimatedMinutes:number(f,"EstimatedMinutes"),splittable:boolean(f,"IsSplittable"),progress:status==="Completada"?100:number(f,"Progress"),assignedUserId:requiredText(f,"AssignedUserId")}}),
  ...
  users:(items:SharePointListItem[])=>mapItems<User>("CW_Users",items,f=>({id:requiredText(f,"AppId"),name:requiredText(f,"Title"),email:requiredText(f,"Email").toLowerCase(),initials:requiredText(f,"Initials"),role:oneOf(f,"Role",["admin","gestor","usuario"] as const),assignedProjectIds:[],editableProjectIds:[],permissions:{manageUsers:boolean(f,"ManageUsers"),manageProjects:boolean(f,"ManageProjects"),manageSchedule:boolean(f,"ManageSchedule")},active:boolean(f,"IsActive"),entraObjectId:optionalText(f,"EntraObjectId")})),
