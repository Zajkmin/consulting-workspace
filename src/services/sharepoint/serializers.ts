...
export const listSerializers: { [K in SharePointEntityKey]: (entity: SharePointEntityByKey[K]) => Record<string, unknown> } = {
  ...
  users: entity => ({ Title:entity.name,AppId:entity.id,Email:entity.email.trim().toLowerCase(),Initials:entity.initials,Role:entity.role,ManageUsers:entity.permissions?.manageUsers??false,ManageProjects:entity.permissions?.manageProjects??false,ManageSchedule:entity.permissions?.manageSchedule??false,CanCreateProjects:entity.permissions?.createProjects??false,CanEditProjects:entity.permissions?.editProjects??false,CanDeleteProjects:entity.permissions?.deleteProjects??false,CanCreateInitiatives:entity.permissions?.createInitiatives??false,CanEditInitiatives:entity.permissions?.editInitiatives??false,CanDeleteInitiatives:entity.permissions?.deleteInitiatives??false,CanCreateVersions:entity.permissions?.createVersions??false,CanEditVersions:entity.permissions?.editVersions??false,CanDeleteVersions:entity.permissions?.deleteVersions??false,CanCreateTasks:entity.permissions?.createTasks??false,CanEditTasks:entity.permissions?.editTasks??false,CanDeleteTasks:entity.permissions?.deleteTasks??false,CanViewOthersTasks:entity.permissions?.viewOthersTasks??true,CanCreateUsers:entity.permissions?.createUsers??false,CanEditUsers:entity.permissions?.editUsers??false,CanDeleteUsers:entity.permissions?.deleteUsers??false,IsActive:entity.active,EntraObjectId:entity.entraObjectId??null }),
