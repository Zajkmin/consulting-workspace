...
export const listSerializers: { [K in SharePointEntityKey]: (entity: SharePointEntityByKey[K]) => Record<string, unknown> } = {
  ...
  versions: entity => ({ Title:entity.name,AppId:entity.id,InitiativeId:entity.initiativeId,Code:entity.code,Status:entity.status,OwnerUserId:entity.ownerUserId,StartDate:entity.startDate,Deadline:entity.deadline,Validated:entity.validated }),
  ...
  users: entity => ({ Title:entity.name,AppId:entity.id,Email:entity.email.trim().toLowerCase(),Initials:entity.initials,Role:entity.role,ManageUsers:entity.role==="admin",ManageProjects:entity.role==="admin"||entity.role==="gestor",ManageSchedule:true,IsActive:entity.active,EntraObjectId:entity.entraObjectId??null }),
